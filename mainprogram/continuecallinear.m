...
%stressyx0=double(savefield(48).tyx);
%stressyz0=double(savefield(48).tyz);
...
savelist={'earthquakecount','elapsedtime','equ','faultdepth','faultdepth0','interseismiccount','intertime','next','next0','ii','j','jj'};
...
[etaeff,esyxv,esyzv]=stress2visco(Fdisl,model.Mesh,stressyx0,stressyz0);
rej=j+1;
[A,B]=createAB(p,t);
AB=B*A;
[A,B]=createBA(p,t);
...
for j=rej:loops
    ...
    if next0~=next
        if next % switch from coseismic to interseismic
            ...
            if ii~=1 % if ii=1, data already saved
                if exist([savefolder filesep num2str(earthquakecount,'%04.0f') '_' num2str(jj*100+ii,'%04.0f') '_lineardata.mat'],'file')
                    error('file exist')
                else
                    disp(['Saving data to' savefolder filesep num2str(earthquakecount,'%04.0f') '_' num2str(jj*100+ii,'%04.0f') '_lineardata.mat'] )
                    % save data struct and loop information
                    save([savefolder filesep num2str(earthquakecount,'%04.0f') '_' num2str(jj*100+ii,'%04.0f') '_lineardata.mat'],'savefield',savelist{:},'-v7.3');
                    disp('Data saved')
                end
            end
            % reset
    ...
    if next
        ...
        [stressyx,stressyz,elapsedtime,v,dt]=stressaccumulationplinear(stressyx,stressyz,etaeff,elapsedtime,model,femodelinter);
...
%         reset(gpuDev);
    ...
    if ii==100
        ...
        jj=jj+1;
        if exist([savefolder filesep num2str(earthquakecount,'%04.0f') '_' num2str(jj*ii,'%04.0f') '_lineardata.mat'],'file')
            error('file exist')
        else
            disp(['Saving data to ' savefolder filesep num2str(earthquakecount,'%04.0f') '_' num2str(jj*ii,'%04.0f') '_lineardata.mat'])
            %         save([savefolder filesep num2str(earthquakecount,'%04.0f') '_' num2str(jj*ii,'%04.0f') '.mat'],'-v7.3');
            % save data struct
            save([savefolder filesep num2str(earthquakecount,'%04.0f') '_' num2str(jj*ii,'%04.0f') '_lineardata.mat'],'savefield',savelist{:},'-v7.3');
            disp('Data saved')
        end
